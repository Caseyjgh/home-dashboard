...
export function ImportantEvents() {
  ...
  useEffect(() => {
    ...
    if (!parent) return;
    const observer = new ResizeObserver(() => setLimit(Math.max(1, Math.floor((parent.clientHeight * .4 - 44) / 72))));
  ...
  return <div ref={anchor} className={`important-events ${entries.length ? "" : "empty"}`}>
    {entries.length > 0 && <section aria-label="Important reminders"><ul>{entries.slice(current * limit, (current + 1) * limit).map((entry) => <li key={entry.id} className={entry.highImportance ? "high-importance" : ""}>
