...
Weather is fixed to Windsor, Colorado (40.4775, -104.9014), with America/Denver timezone. No weather environment variables or API key are used. `/api/weather` calls [Open-Meteo](https://open-meteo.com/en/docs) server-side for current conditions and seven daily forecasts with Fahrenheit, mph and inches. The card shows current temperature/condition, today's high/low and precipitation probability, plus the following five days. The header and card share one request and timer. Upstream fetches revalidate every 15 minutes; visible browsers request the cached endpoint at that interval. Hidden/offline tabs pause, failures keep the last forecast with a saved marker, and a cold-load outage shows a small unavailable message without blocking the dashboard. The old WEATHER_LOCATION setting is no longer read.
